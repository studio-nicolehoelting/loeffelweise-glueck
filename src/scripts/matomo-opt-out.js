var settings = {
    ...
    YouAreOptedOut:
        "Ihr Besuch dieser Webseite wird aktuell von der Matomo Webanalyse nicht erfasst.",
...
function showContent(consent, errorMessage = null, useTracker = false) {
    ...
    if (!div) {
        ...
        var msg =
            'Unable to find opt-out content div: "' + settings.divId + '"';
    ...
    if (consent) {
        if (settings.showIntro) {
            // content +=
            //     "<p>" +
            //     settings.YouMayOptOut2 +
            //     " " +
            //     settings.YouMayOptOut3 +
            //     "</p>";
            let showIntro = document.createElement("p");
            showIntro.innerHTML =
                settings.YouMayOptOut2 + " " + settings.YouMayOptOut3;
            content.push(showIntro);
        }
        if (useTracker) {
            // content +=
            //     '<input onclick="_paq.push([\'optUserOut\']);showContent(false, null, true);" id="trackVisits" type="checkbox" checked="checked" />';
            let useTracker = document.createElement("input");
            useTracker.addEventListener("click", () => {
        ...
        if (settings.showIntro) {
            // content +=
            //     "<p>" +
            //     settings.OptOutComplete +
            //     " " +
            //     settings.OptOutCompleteBis +
            //     "</p>";
            let showIntro = document.createElement("p");
            showIntro.innerHTML =
